...
// ✅ New components for Kits
import AdminKitManagement from "./components/AdminKitManagement";
import StudentKits from "./components/StudentKits";
...
export default function DashboardPage() {
  ...
  const sidebarItems = user.role === "ADMIN"
    ? [
        ...
        { title: "Kits", key: "kits", icon: IconBox }, // ✅ Admin kits
    ...
    : [
        ...
        { title: "My Kits", key: "myKits", icon: IconBox }, // ✅ Student kits
      ];
  ...
  const renderContent = () => {
    if (user.role === "ADMIN") {
      switch (activeView) {
        ...
        case "kits": return <AdminKitManagement />; // ✅ Admin manages kits
      ...
      switch (activeView) {
        ...
        case "myKits": return <StudentKits />; // ✅ Student sees kits
        default: return <p>Not Found</p>;
      }
    }
  };
  ...
  return (
    <div className="flex">
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-screen bg-background border-r border-border transition-all duration-300 flex flex-col ${
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto p-2 space-y-2">
          {sidebarItems.map((item) => (
            <button
              key={item.key}
              className={`flex items-center gap-2 p-2 rounded w-full hover:bg-accent/10 ${
                activeView === item.key ? "bg-accent/20 font-semibold" : ""
              }`}
      ...
      {/* Main Content */}
      <main
        className={`flex-1 p-4 md:p-6 overflow-y-auto h-screen transition-all duration-300 ${
